package com.tscp.mvne.billing.system;
...
import com.tscp.mvne.billing.util.MessageUtil;
...
public class BillingSystem implements BillingInterface {
  ...
  private static void loadDefaults() {
    ...
    try {
      ...
      BillingSystem.SERVICE_INSTANCE_HOTLINED = props.getProperty("service.hotlined", BillingSystem.SERVICE_INSTANCE_HOTLINED);
      BillingSystem.SERVICE_INSTANCE_RESTORED = props.getProperty("service.restored", BillingSystem.SERVICE_INSTANCE_RESTORED);
  ...
  public void addComponent(int accountNumber, ServiceInstance serviceInstance, Package iPackage, Component component)
      throws BillingException {
    if (accountNumber <= 0) {
      throw new BillingException("addServiceInstance", "Error adding service to unknown Account");
    }
    if (serviceInstance == null || serviceInstance.getExternalId() == null || serviceInstance.getExternalId().trim().isEmpty()) {
    ...
    if (component != null && component.getComponentId() > 0) {
      pkgComponent.setComponentId(component.getComponentId());
    } else {
      component.setComponentId(pkgComponent.getComponentId());
    }
      ...
      for (MessageHolder message : messageHolder.getMessageHolder()) {
        if (message.getStatus().equals("Success")) {
          ...
          throw new BillingException("addPackage", "Error adding component " + pkgComponent.getComponentId() + " to Account "
              + accountNumber + ". Returned message is " + message.getMessage());
        }
      }
    }
  }
  ...
  public void addPackage(Account account, ServiceInstance serviceInstance, Package iPackage) throws BillingException {
    addPackage(account.getAccountno(), serviceInstance, iPackage);
  }

  public void addPackage(int accountNumber, ServiceInstance serviceInstance, Package iPackage) throws BillingException {
    if (accountNumber <= 0) {
      throw new BillingException("addServiceInstance", "Error adding service to unknown Account");
    }
    if (serviceInstance == null || serviceInstance.getExternalId() == null || serviceInstance.getExternalId().trim().isEmpty()) {
    ...
    kenanPackage.setAccountNo(Integer.toString(accountNumber));
    ...
    if (valueHolder.getValueHolder() != null && valueHolder.getValueHolder().size() > 0) {
      for (ValueHolder value : valueHolder.getValueHolder()) {
        if (value.getStatusMessage().getStatus().equals("Success")) {
          System.out.println("Added Package " + iPackage.getPackageid() + " :: Kenan returned ID -> " + value.getValue().trim()
          ...
          throw new BillingException("addPackage", "Error adding package " + kenanPackage.getPackageId() + " to Account "
              + accountNumber + ". Returned message is " + value.getStatusMessage().getMessage());
        }
      }
    }
  }
  ...
  public void addPayment(Account account, String paymentAmount) throws BillingException {
    ...
    try {
      ...
      e.printStackTrace();
      throw new BillingException("addPayment", "Unable to add payment to Billing Server");
    }
  }
  ...
  public void deleteServiceInstance(Account account, ServiceInstance serviceinstance) throws BillingException {
    removeServiceInstance(account.getAccountno(), serviceinstance);
  }
  ...
  public List<PaymentHolder> getCompletePaymentHistory(Account account) {
    ArrayOfPaymentHolder paymentHolderList = port.getCompletePaymentHistory("username", Integer
        .toString(account.getAccountno()));
    ...
    try {
      ...
      if (arrayOfPackages != null) {
        ...
        for (PackageHolder packageHolder : arrayOfPackages.getPackageHolder()) {
          ...
          tscpPackage.setPackage_instance_id_serv(Short.toString(packageHolder.getPackage().getPackageInstanceIdServ()));
  ...
  /**
   * Returns the ServiceInstance for the given DeviceInfo. DeviceInfo should be
   * the business object that drives the backend requests.
   * 
   * @param deviceInfo
   * @return
   * @throws BillingException
   */
  public ServiceInstance getServiceInstance(DeviceInfo deviceInfo) throws BillingException {
    List<ServiceInstance> services = getServiceInstances(deviceInfo.getAccountNo());
    if (services != null) {
      for (ServiceInstance si : services) {
        System.out.println("TSCPMVNE! found service " + si.getExternalId() + " for account " + deviceInfo.getAccountNo());
      }
    } else {
      System.out.println("TSCPMVNE! no services found - null");
    }
    for (ServiceInstance service : services) {
      if (service.getExternalId().equals(deviceInfo.getNetworkInfo().getMdn())) {
        return service;
      }
    }
    return null;
  ...
  public List<ServiceInstance> getServiceInstanceList(Account account) throws BillingException {
    return getServiceInstances(account.getAccountno());
  }

  public List<ServiceInstance> getServiceInstances(int accountNumber) throws BillingException {
    if (accountNumber <= 0) {
      throw new BillingException("getServiceInstanceList", "Account information must be populated.");
    }
    try {
      ArrayOfServiceHolder serviceHolderList = port.getActiveService("username", Integer.toString(accountNumber));
      ...
      System.out.println("WS Exception thrown when calling getActiveService(\"username\"," + accountNumber + ")...."
          + ws_ex.getMessage());
  ...
  public void removeServiceInstance(int accountNumber, ServiceInstance serviceInstance) throws BillingException {
    System.out.println("Disconnecting Service on Account " + accountNumber + " and ServiceInstance "
    ...
    MessageHolder message = port.disconnectServicePackages("username", Integer.toString(accountNumber), serviceInstance
    ...
    if (message == null) {
      ...
      if (!message.getStatus().equals("Success")) {
        throw new BillingException("deleteServiceInstance", "Error deleting ServiceInstance " + serviceInstance.getExternalId()
            + " to account " + accountNumber + "..." + message.getMessage());
      }
    }
    System.out.println("Done Disconnecting Service on Account " + accountNumber + " and ServiceInstance "
        + serviceInstance.getExternalId());
  }
  ...
  public void updateAccount(Account account) throws BillingException {
    ...
    if (account.getContact_email() != null) {
      MessageHolder message = port
          .updateEmail("username", Integer.toString(account.getAccountno()), account.getContact_email());
      if (message == null) {
  ...
  public void updateEmailAddress(int accountNumber, String email) {
    if (accountNumber <= 0) {
      throw new BillingException("Account number cannot be empty");
    } else if (email == null || email.trim().isEmpty()) {
      throw new BillingException("Email cannot be empty");
    }
    MessageHolder messageHolder = port.updateEmail("system", Integer.toString(accountNumber), email);
    if (messageHolder != null) {
      System.out.println(MessageUtil.getStatus(messageHolder));
    } else {
      throw new BillingException("No response from billing system.");
    }
  }
